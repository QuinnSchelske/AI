...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        for g in successorGameState.getGhostPositions():
			xy2 = g
			ghostDist += abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
			if ghostDist == 0:
				return -100000
        foodDist=0
        for f in newFood.asList():
			xy2 = f
			foodDist += abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
        #print ghostDist
        #print foodDist
        cDist = 0
        ...
        for c in successorGameState.getCapsules():
			xy2 = c
			cDist += abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
        if newScaredTimes[0]>5:
			func = -foodDist 
        else:
			func = successorGameState.getScore()*20 + 5*ghostDist -foodDist -cDist*10
        return func
